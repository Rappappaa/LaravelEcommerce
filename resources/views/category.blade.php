                ...
                <div class="page-header__breadcrumb">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            @if($uppercategory)
                            <li class="breadcrumb-item">
                                <a href="{{ route('shop.maincategory',$uppercategory->slug) }}">{{ $uppercategory->name }}</a>
                                <svg class="breadcrumb-arrow" width="6px" height="9px">
                                    <use xlink:href="../images/sprite.svg#arrow-rounded-right-6x9"></use>
                                </svg>
                            </li>
                                                    ...
                                                    <div class="filter__container">
                                                        <div class="filter-categories">
                                                            <ul class="filter-categories__list">
                                                                @if($category->upperid == null)
                                                                    @foreach($sub_categories as $sub_category)
                                                                        <li class="filter-categories__item filter-categories__item--parent">
                                                                            <a href="{{ route('shop.maincategory', $sub_category->slug) }}">{{ $sub_category->name }}</a>
                                                                            <div class="filter-categories__counter">
                                                                                @php
                                                                                    $items = DB::table('product_category')->join('product','product.id','=','product_category.ref_product')->where('ref_category',$sub_category->id)->where('product.active','=',true)->Count();
                                                                                    echo $items;
                                                                                @endphp
                                                                ...
                                                                @else
                                                                    @foreach($menu as $menu)
                                                                        @if($menu->active == true)
                                                                            <li class="filter-categories__item filter-categories__item--parent">
                                                                                <a href="{{ route('product',$menu->slug) }}">{{ $menu->name }}</a>
                                                                                <div class="filter-categories__counter">
                                                                                    @php
                                                                            ...
                                                                            @php
                                                                                if($category->upperid == null){
                                                                                    $weightsCount = DB::table('category')
                                                                                                ->where('category.upperid','=',$category->id)
                                                                                                ->where('product.weight','=',$weight->weight)
                                                                                                ->join('product_category','product_category.ref_category','=','category.id')
                                                                                                ->join('product','product.id','=','product_category.ref_product')
                                                                                                ->where('product.active','=',true)
                                    ...
                                    @foreach($products as $product)
                                        @foreach($sub_categories as $sub_category)
                                            @foreach($product_categories as $product_category)
                                                @if($product_category->ref_category == $sub_category->id && $product_category->ref_product == $product->id)
        ...
        @else
            <div class="container">
                <div style="horiz-align: center; text-align: center;">
                    Bu kategoriye ait ürün bulunmamaktadır.<br/><br/>
                    <a href="{{ url()->previous() }}" class="btn btn-primary cart__update-button">Geri Dön</a><br/><br/><br/><br/>
                </div>
